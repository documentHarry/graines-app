...
export function enregistrerUtilisateurRoleIpc(
  utilisateurRoleRepository: UtilisateurRoleRepository
): void {
  ipcMain.handle('utilisateur-roles:get-by-utilisateur', async (_event,
    idUtilisateur: number) => {
      return utilisateurRoleRepository.getByUtilisateur(idUtilisateur);
    }
  );

  ipcMain.handle('utilisateur-roles:update', async (_event,
    donnees: {
